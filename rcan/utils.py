...
def convert_to_multi_gpu_model(model, gpus=None):
    """
    TF2.10 note:
    - keras.utils.multi_gpu_model() is not available in tf.keras.
    - In TF2, use tf.distribute.MirroredStrategy and build/compile the model
      inside strategy.scope().
    For now, we return the original model unchanged.
    """
    return model
...
def apply(model, data, overlap_shape=None, verbose=False):
    ...
    num_input_channels = int(model.input.shape[-1])
    num_output_channels = int(model.output.shape[-1])
