...
class DataGenerator:
    ...
    def __init__(self,
        ...
        # self._scale_factor = _normalize_tuple(scale_factor, dim, 'scale_factor')
    ...
    def _normalize_tuple(value, n, name):
        if isinstance(value, int):
            return (value,) * n
        if isinstance(value, (tuple, list)) and len(value) == n and all(isinstance(v, int) for v in value):
            return tuple(value)
        raise ValueError(f'"{name}" must be an int or a tuple/list of {n} ints')
    ...
    def flow(self, x, y, steps_per_epoch=256):
        '''
