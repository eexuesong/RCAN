...
def load_data(config, data_type):
    ...
    if data_type + '_data_dir' in config:
        raw_dir = pathlib.Path(config[data_type + "_data_dir"]["raw"])
        gt_dir = pathlib.Path(config[data_type + "_data_dir"]["gt"])
        ...
        if len(raw_files) != len(gt_files):
            raise RuntimeError(
                f'"{raw_dir}" and "{gt_dir}" must contain the same number of TIFF files')
...
def main():
    ...
    schema = {
        ...
        "anyOf": [{"required": ["training_image_pairs"]}, {"required": ["training_data_dir"]}],
        "definitions": {
            "raw_gt_pair": {
                "type": "object",
                "properties": {"raw": {"type": "string"}, "gt": {"type": "string"}}
            },
            "image_pairs": {
                "type": "array",
    ...
    if training_data is None:
        raise RuntimeError("No training data found. Check your config paths.")
    ...
    if "input_shape" in config:
        ...
        if len(input_shape) != ndim:
            raise ValueError(f"`input_shape` must be a {ndim}D array; received: {input_shape}")
    else:
        input_shape = (16, 256, 256) if ndim == 3 else (256, 256)

    # Ensure patch fits all images.
    for p in itertools.chain(training_data, validation_data or []):
        input_shape = tuple(np.minimum(input_shape, p[0].shape))

    print("Building RCAN model")
    print("  - input_shape =", input_shape)
    for s in ["num_channels", "num_residual_blocks", "num_residual_groups", "channel_reduction"]:
        print(f"  - {s} =", config[s])
    ...
    #     input_shape,
    #     gpus,
    #     transform_function=("rotate_and_flip" if config["data_augmentation"] else None),
    #     intensity_threshold=config["intensity_threshold"],
    #     area_ratio_threshold=config["area_ratio_threshold"])

    data_gen = DataGenerator(
        input_shape,
        batch_size=1,  # single-GPU, keep it explicit
        transform_function=("rotate_and_flip" if config["data_augmentation"] else None),
        intensity_threshold=config["intensity_threshold"],
        area_ratio_threshold=config["area_ratio_threshold"])
    ...
    model.fit(
        ...
        verbose=0,
        callbacks=[
            keras.callbacks.LearningRateScheduler(staircase_exponential_decay(max(1, config["epochs"] // 4))),
            keras.callbacks.TensorBoard(log_dir=str(output_dir), write_graph=False),
            ModelCheckpoint(
